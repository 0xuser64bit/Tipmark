...
/**
 * The editor screen, shared by `/claim` and `/edit-profile`.
 *
 * They are one screen with two URLs on purpose: the connected wallet decides
 * whether opening it means creating or updating, but "claim" is the link worth
 * sharing and "edit profile" is the one a creator looks for later. Only the
 * page title differs, so the body lives here rather than in both routes.
 */
export async function ProfileEditorRoute({
  searchParams,
}: {
